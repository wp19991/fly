...
# 创建一个UDP连接
master = mavutil.mavlink_connection('udpout:localhost:14540')
...
while True:
    ...
    integrated_x = random.uniform(-1.5, 1.5)  # 光流x向量（rad）
    integrated_y = random.uniform(-2.5, 2.5)  # 光流y向量（rad）
    ...
    distance = random.uniform(5, 15)  # 距离传感器的距离，以米为单位
